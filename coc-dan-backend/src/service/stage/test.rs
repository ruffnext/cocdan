...
async fn test_stage_basic() {
    ...
    let response = server
        ...
        .json(&ReqCreateStage {
            ...
            rule: StageRule::CoC7th,
        })
        .add_cookie(session.clone())
        .await;
    assert!(response.status_code() == StatusCode::OK);

    let new_stage: Stage = response.json();
    assert!(new_stage.title == title);
    assert!(new_stage.description == description);

    // get stage
    let response = server
        .get(format!("/api/stage/{}", new_stage.raw_id).as_str())
        .add_cookie(session.clone())
        .await;

    let get_stage: Stage = response.json();
    assert!(get_stage.raw_id == new_stage.raw_id);
    assert!(get_stage.title == title);

    // remove a stage
    let response = server
        .post(format!("/api/stage/{}/remove", new_stage.raw_id).as_str())
        .add_cookie(session.clone())
        .await;
    assert!(response.status_code() == StatusCode::OK);
    ...
    // // get stage by id
    // let response = server
    //     .get(format!("/api/stage/{}", new_stage.id).as_str())
    //     .add_cookie(session.clone())
    //     .await;
    ...
    // // get stage users
    // let res = server
    //     .get(format!("/api/stage/{}/users", new_stage.id).as_str())
    //     .add_cookie(session.clone())
    //     .await;
    ...
    // let res = server
    //     .post(format!("/api/stage/{}/join", first_stage.id).as_str())
    //     .add_cookie(session2.clone())
    //     .await;
    ...
    //     let res = server
    //         .post(format!("/api/stage/{}/leave", user_3_stage.id).as_str())
    //         .add_cookie(session.clone())
    //         .await;
    ...
    // // user 3 leaves
    // let res = server
    //     .post(format!("/api/stage/{}/leave", new_stage.id).as_str())
    //     .add_cookie(session3.clone())
    //     .await;
    // assert!(res.status_code() == StatusCode::OK);
    // assert!(
