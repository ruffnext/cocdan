...
pub async fn get_realtime_state_write_lock(
) -> RwLockWriteGuard<'static, HashMap<i32, RwLock<RealtimeState>>> {
...
pub async fn lock_stage_state<T>(stage_id: i32, db: &T) -> Result<StageStateLock, Left>
where
    T: ConnectionTrait,
{
    let state: &'static RwLock<HashMap<i32, RwLock<RealtimeState>>> =
        STAGE_STATE.get_or_init(init_state).await;
    let global_lock = state.read().await;
    let global_read_lock: RwLockReadGuard<'static, HashMap<i32, RwLock<RealtimeState>>> =
        if global_lock.contains_key(&stage_id) == false {
            drop(global_lock);
