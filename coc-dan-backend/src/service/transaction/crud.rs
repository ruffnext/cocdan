...
use coc_dan_common::def::transaction::{service::IQueryStageTxs, ITransaction, Tx};
...
pub async fn query_stage_txs(
    ...
    let mut query =
        transaction::Entity::find().filter(transaction::Column::StageId.eq(stage_user.stage.id));
    match params.begin {
        Some(v) => query = query.filter(transaction::Column::TxId.gte(v)),
        None => {}
    }
    match params.end {
        Some(v) => query = query.filter(transaction::Column::TxId.lte(v)),
        None => {}
    }
    ...
    let stage_id = match user_avatar.avatar.stage_id {
        ...
        None => {
            return Err(Left {
                ...
                message: format!(
                    "Avatar {} is not on a stage, no transaction needed",
