use coc_dan_common::def::{
    avatar::{service::ICreateAvatar, IAvatar},
    stage::IStage,
};
use http::StatusCode;
use serde_json::json;
...
async fn test_avatar_basic() {
    ...
    // delete an avatar
    let res = server
        .delete(format!("/api/avatar/{}", avatar_0.id).as_str())
        .add_cookie(session.clone())
        .await;
    assert_eq!(res.status_code(), StatusCode::OK);
    ...
    let res = server
        .post(format!("/api/stage/{}/leave", stage_0.id).as_str())
        .add_cookie(session.clone())
        .await;
    assert_eq!(res.status_code(), StatusCode::OK);
