...
use sea_orm::{
    ...
    IntoActiveModel, PaginatorTrait, QueryFilter, QuerySelect, TransactionTrait,
};
...
pub async fn get_by_id_req(
    ...
    Ok((
        http::StatusCode::OK,
        Json(get_avatar_by_id(u, Path(id), State(state)).await?),
    )
        .into_response())
}
...
pub async fn create(
    ...
    let res = db
        .transaction::<_, IAvatar, DbErr>(|ctx| {
            Box::pin(async move {
                let res: IAvatar = avatar::ActiveModel {
                    stage_id: ActiveValue::Set(params.stage_id),
                    owner: ActiveValue::Set(u.id),
                    name: ActiveValue::Set(params.name),
                    detail: ActiveValue::Set(
                        serde_json::to_string(&params.detail.unwrap_or_default()).unwrap(),
                    ),
            ...
            Box::pin(async move {
                match user_avatar.avatar.stage_id {
                    Some(v) => {
                        ...
                        let tx = Tx::UpdateAvatar {
                            before: Some(user_avatar.avatar.into()),
                            after: None,
                        };
                        perform_tx((&mut state, v), ctx, user_avatar.user.id, avatar_id, &tx)
                            .await?;
                    }
                    None => {
                        Avatar::delete_by_id(user_avatar.avatar.id)
                            .exec(ctx)
                            .await?;
                    }
                };
...
pub async fn update_avatar(
    ...
    state
        ...
        .transaction::<_, (), DbErr>(|ctx| {
            ...
            Box::pin(async move {
                update_avatar_inner(
                    user_avatar.user.into(),
                    user_avatar.avatar.into(),
                    a.into(),
                    ctx,
                )
                .await?;
    ...
    let res_tx = match (avatar_before.stage_id, avatar_after.stage_id) {
        (None, None) => {
            ...
            ITransaction {
                ...
                tx: Tx::UpdateAvatar {
                    before: Some(avatar_before),
                    after: Some(avatar_after),
                },
            }
        }
        (Some(a), Some(b)) => {
