...
impl MigrationTrait for M241028Init {
    ...
    async fn setup(&self, db: &super::DbConn) -> Result<(), surrealdb::Error> {
        ...
        DEFINE FIELD raw_id ON user TYPE string;
        ...
        DEFINE FIELD raw_id ON TABLE stage TYPE string;
        ...
        DEFINE FIELD raw_id ON TABLE tx TYPE string;
        DEFINE FIELD tx_index ON TABLE tx TYPE int;
