...
impl State {
    pub fn apply_tx_rev(&mut self, tx: &ITransaction) -> bool {
        ...
        match &tx.tx {
            ...
            Tx::Dice(dice) => match &dice.result {
    ...
    /// return bool indicates whether state changes
    ...
    pub fn query_state(&self, idx: usize) -> Result<State, Left> {
        if idx > self.idx || idx < self.idx_base {
            return Err(Left {
                ...
                message: format!(
                    "query state out of range, idx range in ({}, {}), but query is {}",
                    self.idx_base, self.idx, idx
                ),
                uuid: "c4a28ada",
            });
        };
