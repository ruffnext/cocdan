...
#[ts(
    ...
    export_to = "bindings/avatar/ISkillCategory.ts"
)]
...
#[ts(
    ...
    export_to = "bindings/avatar/IOptionalOccupationalSkill.ts"
)]
...
#[ts(
    ...
    export_to = "bindings/avatar/IOccupationalSkill.ts"
)]
#[serde(untagged)]
...
#[ts(
    export,
    rename = "IOccupation",
    export_to = "bindings/avatar/IOccupation.ts"
)]
...
lazy_static! {
    pub static ref SKILLS: HashMap<String, Skill> =
        serde_json::from_str::<Vec<Skill>>(include_str!("../../resource/skills.json"))
            .unwrap()
            .into_iter()
            .map(|x| (x.name.to_string(), x))
            .collect();
    pub static ref OCCUPATIONS: HashMap<String, Occupation> =
        serde_json::from_str::<Vec<Occupation>>(include_str!("../../resource/occupation.json"))
            .unwrap()
            .into_iter()
            .map(|x| (x.name.to_string(), x))
            .collect();
...
#[ts(
    ...
    export_to = "bindings/avatar/ISkillAssigned.ts"
)]
