...
export function setSkill(item: ISkillAssigned, avatar: IAvatar, setAvatar: SetStoreFunction<IAvatar>): boolean {
  ...
  if (item.name == "CreditRating" && total < avatar.version.occupation.credit_rating[0]) return false
  if (item.name == "CreditRating" && total > avatar.version.occupation.credit_rating[1]) return false
  if (item.interest_skill_point < 0 || item.occupation_skill_point < 0) return false

  for (const key in avatar.version.skills) {
    const val = avatar.version.skills[key]
