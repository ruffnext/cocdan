...
(deftest ops-test
  (let [avatars [{:id "avatar-1" :name "avatar-name" :image nil :description "description" :controlled_by "user-1"}]
        ...
        op1 (core-ops/make-transaction 1 0 1 core-ops/OP-SNAPSHOT stage)
        _work (d/transact! db (core-ops/ctx-generate-ds "stage-1" op1 nil))
        ...
        op2 (core-ops/make-transaction 2 1 2 core-ops/OP-UPDATE op2-diffs)
        _work (d/transact! db (core-ops/ctx-generate-ds "stage-1" op2 ctx1))
        ...
        op3 (core-ops/make-transaction 3 2 3 "speak" {:type :speak :avatar "avatar-1" :payload {:message "hello" :props {}}})]
    (testing "basic test"
      (is (= ctx1 ctx1-real))
      (is (= ctx2 ctx2-real)))
    (core-ops/ctx-generate-ds db op3)))